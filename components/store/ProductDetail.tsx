    ...
    <>
      ...
      <div className="pdp">
        ...
        <div className="pdp-gallery">
          <div className="thumbs">
            {(images.length > 0 ? images : [null, null, null, null]).slice(0,4).map((img, i) => (
              <div key={i} className={`thumb ${activeImg === i ? 'active' : ''}`}
                ...
                {img ? (
                  <Image src={optimizeImage(img, {width:200})} alt={`${p.name} ${i+1}`} fill sizes="90px"
          ...
          >
            {images[activeImg] ? (
              <Image src={optimizeImage(images[activeImg], {width:1200, quality:80})} alt={p.name} fill priority
