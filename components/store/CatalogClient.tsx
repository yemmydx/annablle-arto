    ...
    <div className="card" style={{cursor:'pointer'}}>
      <div className="card-img" style={{background: CARD_BG[gi]}} onClick={() => window.location.href = `/product/${product.slug}`}>
        {product.images && product.images.length > 0 ? (
          <>
            <Image src={optimizeImage(product.images[0], {width:600})} alt={product.name} fill
              sizes="(max-width:640px) 50vw, (max-width:900px) 33vw, 25vw"
              style={{objectFit:'cover'}} />
            {product.images[1] && (
              <Image src={optimizeImage(product.images[1], {width:600})} alt={product.name} fill className="card-img-hover"
                sizes="(max-width:640px) 50vw, (max-width:900px) 33vw, 25vw"
