    ...
    <div className="card" style={{cursor:'pointer'}}>
      <div className="card-img" style={{background: (product.images && product.images.length>0)?'#f6ede8':CARD_BG[gi]}} onClick={() => window.location.href = `/product/${product.slug}`}>
        {product.images && product.images.length > 0 ? (
          <>
            <Image src={optimizeImage(product.images[0], {width:1000, quality:90})} alt={product.name} fill
              sizes="(max-width:640px) 50vw, (max-width:900px) 33vw, 25vw"
              style={{objectFit:'contain'}} />
            {product.images[1] && (
              <Image src={optimizeImage(product.images[1], {width:1000, quality:90})} alt={product.name} fill className="card-img-hover"
                sizes="(max-width:640px) 50vw, (max-width:900px) 33vw, 25vw"
